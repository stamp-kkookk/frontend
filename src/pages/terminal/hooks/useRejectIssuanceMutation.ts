...
import { rejectIssuance } from '../api/terminal';

export const useRejectIssuanceMutation = (storeId: string) => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (requestId: string) => rejectIssuance({ storeId, requestId }),
    onSuccess: () => {
      // 거절 성공 시, 관련 쿼리를 무효화하여 데이터를 새로고침합니다.
